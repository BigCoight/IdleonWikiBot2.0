...
COLNAMES = ["Dewdrop", "Sandstone", "Frostbite",
...
def writeShopsJSON():
    ...
    shopsLocations = [MAPNAMES[int(x)] for x in re.findall(
...
def writeEnemiesJSON():
    ...
    for i in range(1, len(enemiesData), 2):
        ...
        if intName[:5] == "Chest":  # If we need to change the name of the cols
            enemies[intName]["Name"] = changeChestNames(
                intName, enemies[intName]["Name"])
...
def writePostOfficeJSON():
    ...
    postOfficeData = {}
    postOffices = [
        '[[' + x + ']]' for x in re.split(r'\],?\],?],\[\[\[', postData)]
    for j, postOffice in enumerate(postOffices):
        if j >= len(postNames):
            break
        category = [
            '[[' + x + ']]' for x in re.split(r',?\],?],\[\[', postOffice)]
        temp = {}
            ...
            if bubData[0] not in cauldrens[bubbleNames[n]].keys() and bubData[-1] != "Filler":
                ...
                for i, j in zip([5, 6, 7, 8], [11, 12, 13, 14]):
                    if bubData[i] == "Blank":
                        break
                    if len(bubData) >= 14:
                        cauldrens[bubbleNames[n]][bubData[0]]["requirements"].append(
                            [bubData[i], bubData[j]])
                    else:
                        cauldrens[bubbleNames[n]][bubData[0]
                                                  ]["requirements"].append([bubData[i], "L"])
...
def writeTalentJSON():
    ...
    reEverything = r'"([a-zA-Z0-9_ +{}\',.\-%!$:`?;\n\]\(\)]*)"\.'
    talentOrder = [int(x)
                   for x in strToArray(reader.getSection('TalentOrder'))]
    talentNames = re.findall(reEverything, reader.getSection('TalentNames'))[
        0].split(' ')
    reTalentDesc = r'\[\["(.*)"\], "(.*)"\.split\(" "\), \["(.*)"\], \["(.*)"\]\]'
    talentDescs = [' '.join(x).split(' ') for x in re.findall(
        reTalentDesc, reader.getSection('TalentData'))]
    classNames = re.findall(reEverything, reader.getSection('ClassNames'))[
        0].split(' ')[1:]
    specialTalents = []
...
def writeCardJSON():
    ...
    for n, section in enumerate(['[[' + x + ']]' for x in re.split(r',?\],?],\[\[', cardData)]):
        ...
        for m, data in enumerate(['[[' + x + ']]' for x in re.split(r',?],\[', section)]):
            cardDict[cardNames[n]][strToArray(data)[0]] = strToArray(data)[
                1:] + [str(m)]
...
def writeTaskUnlocks():
    ...
    taskUnlocks = [
        '[[' + x + ']]' for x in re.split(r'\],?\],?],\[\[\[', unlockData)]
    for j, taskUnlock in enumerate(taskUnlocks):
        if j >= 2:
            break
        categories = [
            '[[' + x + ']]' for x in re.split(r',?\],?],\[\[', taskUnlock)]
        temp = []
